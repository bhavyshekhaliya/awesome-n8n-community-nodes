...
| [`n8n-nodes-docmint`](https://www.npmjs.com/package/n8n-nodes-docmint) | Fill DOCX, XLSX, and PPTX templates with workflow data and return the Office file, a PDF, or both; a template's placeholders are read back as typed node fields. | [GitHub](https://github.com/fstandhartinger/n8n-nodes-docmint) | `official` `documents` `docx` |
...
| [`n8n-nodes-pdfmint`](https://www.npmjs.com/package/n8n-nodes-pdfmint) | Render HTML, Markdown, a URL, or a stored template to PDF, render the same input to PNG or JPEG, and merge PDFs, returning the file as binary on the node. | [GitHub](https://github.com/fstandhartinger/n8n-nodes-pdfmint) | `official` `pdf` `documents` |
